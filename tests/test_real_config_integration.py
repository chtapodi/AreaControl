...
def test_real_config_service_driver_all_lights_color():
    ...
    # Simulate HA template light calling pyscript.service_driver_trigger with RGB color and status
    ...
    # Verify that turn_on was called with the RGB color for lights
    color_targets = {
        call[1]["entity_id"]
        for call in color_calls
        if call[0] == "on" and "rgb_color" in call[1] and call[1]["rgb_color"] == [255, 100, 0]
    }
