...
def get_last_track_state(device, scope, *args):
    """Placeholder for legacy tracker compatibility."""
    logger.debug("get_last_track_state(): not implemented for advanced tracker")
    return None
...
def update_tracker(device, *args):
    ...
    logger.debug("update_tracker: state %s", tracker_manager.dump_state())
    ...
    def execute_rule(self, event_data, rule, rule_name=None):
        ...
        if device is not None:
            ...
            logger.info(
                "apply-state",
                device=device_name,
                state=summarize_state(final_state),
            )
            logger.debug(
                f"EventManager:execute_rule(): Applying {final_state} to {scope_names}"
            )
            for areas in scope:
...
def test_tracks() :
    ...
    logger.debug(tracker_manager.dump_state())
